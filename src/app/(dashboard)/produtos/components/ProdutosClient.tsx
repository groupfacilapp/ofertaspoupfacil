...
export function ProdutosClient({
  ...
  return (
    <div className="max-w-7xl space-y-6">
      ...
      {/* ── Fila de Disparo ─────────────────────────────────────────── */}
      <section className="relative overflow-hidden rounded-2xl border border-zinc-800/60 bg-zinc-900/40 backdrop-blur-xl shadow-xl">
        <div className="px-6 py-5 border-b border-zinc-800/60 flex items-center justify-between relative z-10 bg-black/20">
          <div className="flex items-center gap-2">
            <Package className="h-5 w-5 text-indigo-400" />
            <h2 className="text-base font-bold text-white tracking-tight">Fila de Disparo</h2>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center rounded-lg border border-zinc-700/50 bg-zinc-800/40 p-0.5 gap-0.5">
              <button
        ...
        <div className="p-6 space-y-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="rounded-xl border border-zinc-800/40 bg-black/20 backdrop-blur-sm p-4 relative overflow-hidden group">
              ...
              <p className="text-2xl font-bold text-white relative z-10">{currentStats.total}</p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2">
              ...
              {presentMarketplaces.length > 0 && (
                <span className="border-l border-zinc-700/60 mx-0.5 self-stretch" />
              )}
            ...
            <input
              ...
              className="rounded-lg border border-zinc-800 bg-zinc-800/60 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-indigo-500/60 transition-colors w-full sm:w-52"
            />
          </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-indigo-500/20 bg-indigo-600/10 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-xs font-medium text-zinc-400 hover:text-white transition-colors px-3 py-1.5 rounded-lg border border-zinc-700/50 bg-zinc-800/50 hover:border-zinc-600/60"
                >
                  Cancelar
                </button>
                <button
          ...
          {products.length === 0 ? (
                ...
                <button
                  ...
                  className={`flex items-center justify-center h-5 w-5 rounded border transition-all shrink-0 ${
                    selectedIds.size === products.length && products.length > 0
                      ...
                      : 'bg-zinc-800/60 border-zinc-600/60 hover:border-indigo-500/60'
                  }`}
            ...
            <div className="flex flex-col divide-y divide-zinc-800/60 rounded-xl border border-zinc-800/40 overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-2.5 bg-zinc-800/30">
                <button
                  onClick={handleSelectAll}
          ...
          {/* ── Paginação ────────────────────────────────────────────── */}
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-zinc-800/40">
              <p className="text-xs text-zinc-500 order-2 sm:order-1">
                Exibindo <span className="text-zinc-300 font-medium">{itemStart}–{itemEnd}</span> de{' '}
                <span className="text-zinc-300 font-medium">{totalItems}</span> produto(s)
              </p>
              <div className="flex items-center gap-1 order-1 sm:order-2">
                <button
                  onClick={() => navigate({ page: currentPage - 1 })}
                  disabled={currentPage <= 1}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-zinc-700/50 bg-zinc-800/40 text-xs font-medium text-zinc-400 hover:text-white hover:border-zinc-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
                        ...
                        <button
                          ...
                          className={`min-w-[28px] h-7 rounded-lg border text-xs font-medium transition-colors ${
                            currentPage === item
                              ...
                              : 'border-zinc-700/50 bg-zinc-800/40 text-zinc-400 hover:text-white hover:border-zinc-600'
                          }`}
                        >
                          {item}
                        </button>
...
function ProductCard({
  ...
  const mpColor = MARKETPLACE_COLOR[product.marketplace] ?? 'bg-zinc-800/60 text-zinc-400 border border-zinc-700/40';

  return (
    <div className={`relative group overflow-hidden rounded-2xl border bg-zinc-900/40 backdrop-blur-xl transition-all duration-300 hover:border-indigo-500/40 hover:shadow-[0_0_30px_rgba(99,102,241,0.1)] flex flex-col ${isSelected ? 'border-indigo-500/50 shadow-[0_0_20px_rgba(99,102,241,0.15)]' : 'border-zinc-800/60'}`}>
      ...
      {/* Image */}
      <div className="h-44 bg-black/30 relative flex items-center justify-center overflow-hidden border-b border-zinc-800/60 group-hover:bg-black/40 transition-colors">
        {product.image_url ? (
        ...
        <button
          ...
          className={`absolute top-2.5 left-2.5 flex items-center justify-center h-5 w-5 rounded border transition-all z-10 ${
            isSelected
              ...
              : 'bg-zinc-900/80 border-zinc-600/60 opacity-0 group-hover:opacity-100'
          }`}
        ...
        <div className="absolute top-3 right-3 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <a
            ...
            className="flex items-center justify-center h-6 w-6 rounded-md bg-zinc-900/90 text-zinc-400 hover:text-white transition-colors"
            title="Ver produto"
          ...
          <button
            ...
            disabled={isRemoving}
            className="flex items-center justify-center h-6 w-6 rounded-md bg-zinc-900/90 text-zinc-400 hover:text-red-400 transition-colors disabled:opacity-50"
      ...
      <div className="p-4 flex flex-col flex-1 relative z-10">
        ...
        <div className="flex items-baseline gap-2 mb-3">
          <span className="text-lg font-bold text-white tracking-tight">{formatPrice(product.current_price)}</span>
          ...
          return (
            <div className="mb-3">
              <div className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2 py-1 rounded-lg border shadow-sm transition-all ${
                isRecentlySent
                  ? 'bg-zinc-800/40 border-zinc-700/30 text-zinc-500' // Neutral
        ...
        <button
          ...
          className={`w-full text-xs font-semibold tracking-wide rounded-xl py-2.5 transition-all flex items-center justify-center gap-1.5 border shadow-sm ${
            isSent
              ? 'bg-zinc-800/40 border-zinc-700/40 text-zinc-500 cursor-default shadow-none'
              : !hasGroups
              ? 'bg-zinc-800/40 border-zinc-700/40 text-zinc-600 cursor-not-allowed shadow-none'
...
function ProductListItem({
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 transition-colors hover:bg-zinc-800/20 ${isSelected ? 'bg-indigo-600/5' : 'bg-zinc-900/20'}`}>
      {/* Checkbox */}
      <button
      ...
      {/* Image */}
      <div className="shrink-0 h-10 w-10 rounded-lg bg-black/30 border border-zinc-800/60 overflow-hidden flex items-center justify-center">
        {product.image_url ? (
      ...
      <div className="shrink-0 text-right hidden md:block">
        <span className="text-sm font-bold text-white">{formatPrice(product.current_price)}</span>
        {product.discount_percent && product.discount_percent > 0 ? (
      ...
      <div className="shrink-0 flex items-center gap-1.5">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center h-7 w-7 rounded-lg bg-zinc-800/60 border border-zinc-700/40 text-zinc-400 hover:text-white transition-colors"
          title="Ver produto"
          onClick={(e) => e.stopPropagation()}
        >
          <ExternalLink className="h-3 w-3" />
        ...
        <button
          ...
          className={`flex items-center justify-center h-7 px-2.5 rounded-lg border text-xs font-semibold transition-all ${
            isSent
              ? 'bg-zinc-800/40 border-zinc-700/40 text-zinc-500 cursor-default'
              : !hasGroups
        ...
        <button
          onClick={(e) => { e.stopPropagation(); onRemove(); }}
          disabled={isRemoving}
          className="flex items-center justify-center h-7 w-7 rounded-lg bg-zinc-800/60 border border-zinc-700/40 text-zinc-400 hover:text-red-400 transition-colors disabled:opacity-50"
          title="Remover da fila"
        >
          {isRemoving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Trash2 className="h-3 w-3" />}
        </button>
...
function SendPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 w-full max-w-2xl rounded-2xl border border-zinc-700/60 bg-zinc-900 shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/60">
          <div className="flex items-center gap-2">
            <Send className="h-4 w-4 text-indigo-400" />
            <h3 className="text-sm font-semibold text-white">Preview & Enviar</h3>
          </div>
        ...
        <div className="p-5 space-y-5">
          {/* Product summary */}
          <div className="flex gap-3 rounded-xl border border-zinc-800/60 bg-zinc-800/30 p-3">
            <div className="h-14 w-14 shrink-0 rounded-lg bg-zinc-800 overflow-hidden flex items-center justify-center">
              {product.image_url
                ? <img src={product.image_url} alt="" className="h-full w-full object-contain p-1" />
                : <Package className="h-6 w-6 text-zinc-600" />}
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-zinc-200 line-clamp-2 leading-relaxed">{product.title}</p>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${MARKETPLACE_COLOR[product.marketplace] ?? 'bg-zinc-800/60 text-zinc-400'}`}>
                  {MARKETPLACE_LABEL[product.marketplace] ?? product.marketplace}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={8}
              className="w-full rounded-xl border border-zinc-700/60 bg-zinc-800/60 px-4 py-3 text-sm text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-indigo-500/60 transition-colors resize-none font-mono leading-relaxed"
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-4 border-t border-zinc-800/60 bg-zinc-900/80">
          ...
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="rounded-lg border border-zinc-700/60 bg-zinc-800/60 px-4 py-2 text-xs text-zinc-300 hover:text-white transition-colors"
            >
              Cancelar
            </button>
            <button
