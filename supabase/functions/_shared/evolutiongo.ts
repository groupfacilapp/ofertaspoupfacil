...
export class EvolutionGoClient implements WhatsAppClient {
  ...
  async getAllInstances() {
    const res = await this.req<{ data?: Array<{ name?: string; connected?: boolean; qrcode?: string; jid?: string; createdAt?: string }> }>('/instance/all');
    return (res.data ?? []).map(inst => {
      let state: ConnectionState = 'close';
      if (inst.connected) state = 'open';
      else if (inst.qrcode) state = 'connecting';
      const phoneNumber = inst.jid ? inst.jid.split('@')[0] : undefined;
      return { name: inst.name ?? '', state, phoneNumber, createdAt: inst.createdAt };
    });
  }
