...
function findSheinGoodsArrays(obj: unknown, depth = 0): SheinGoodsRaw[][] {
  ...
  if (Array.isArray(obj)) {
    ...
    if (looksLikeGoods && obj.length > 0) found.push(obj as SheinGoodsRaw[]);
    for (const item of obj) found.push(...findSheinGoodsArrays(item, depth + 1));
  } else {
    for (const val of Object.values(obj as Record<string, unknown>)) found.push(...findSheinGoodsArrays(val, depth + 1));
  }
  return found;
}
...
export function generateSheinAffiliateLink(productUrl: string, publisherId: string, websiteId: string, merchantId: string = SHEIN_DEFAULT_CJ_MERCHANT_ID): string {
...
export class SheinConnector implements MarketplaceConnector {
  ...
  async fetchOffers(config: FetchConfig): Promise<NormalizedOffer[]> {
    ...
    for (const g of rawGoods) {
      ...
      const affiliateLink = generateSheinAffiliateLink(productUrl, cj_publisher_id, cj_website_id, merchantId);
      offers.push({ externalId: goodsId, marketplace: 'shein', title, currentPrice, originalPrice, discountPercent, imageUrl, productUrl, affiliateLink, condition: 'Novo', installments: null, category: null, sales: null, couponCode: null });
    }
    return offers;
  }
  ...
  async validateCredentials(credentials: DecryptedCredentials): Promise<ValidationResult> {
    const { cj_publisher_id, cj_website_id } = credentials;
    if (!cj_publisher_id) return { valid: false, error: 'Publisher ID do CJ Affiliate é obrigatório' };
    if (!cj_website_id) return { valid: false, error: 'Website ID do CJ Affiliate é obrigatório' };
    try {
      const response = await fetch('https://br.shein.com/flash-sale.html', { headers: SHEIN_HEADERS });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const html = await response.text();
      const goods = parseSheinNextData(html);
      if (goods.length === 0) return { valid: false, error: 'Não foi possível carregar produtos da Shein.' };
      return { valid: true };
    } catch (e) {
      return { valid: false, error: `Erro de conexão com a Shein: ${e instanceof Error ? e.message : String(e)}` };
    }
  }
}
